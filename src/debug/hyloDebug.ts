...
export class HyloDebugSession extends DebugSession {
  ...
  private async compileFile(filePath: string, args: any): Promise<void> {
    ...
    await this.compileAndRunHylo(
      normalizePath(filePath),
      normalizePath(fileNameWithoutExt),
      args
    );
  }
  ...
  private async findHyloFiles(directoryPath: string): Promise<string[]> {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.sendEvent(
        new OutputEvent(`Error reading directory: ${message}\n`, 'stderr')
      );
    }
   ...
   */
  private async compileAndRunHylo(
    ...
    if (!debugOutputChannel) {
      this.sendEvent(
        new OutputEvent('Output channel not available\n', 'stderr')
      );
      return;
    }
    ...
    const compilerPath =
      args.compilerPath !== undefined
        ? args.compilerPath
        : config.get<string>('compilerPath', 'hc');
    const useCommandTemplate =
      args.useCommandTemplate !== undefined
        ? args.useCommandTemplate
        : config.get<boolean>('useCommandTemplate', false);
    const commandTemplate =
      args.commandTemplate !== undefined
        ? args.commandTemplate
        : config.get<string>('commandTemplate', 'swift run hc ${ARGS}');
    let tempOutputDir =
      args.tempOutputDir !== undefined
        ? args.tempOutputDir
        : config.get<string>('tempOutputDir', '${workspaceFolder}/.hylo_temp');
    ...
    if (args.cwd) {
      ...
      workingDirectory = args.cwd.replace(
        /\${workspaceFolder}/g,
